...
export function NoteEditor({ initialData, onChange, className = '', hideReminder = false, isSettingsOpen = true }: NoteEditorProps) {
    ...
    }, [title, content, tagline, color, isReminder, remindAt, webhookUrl, webhookPayload]);
        ...
        <div className={`relative group transition-colors duration-300 ${getNoteColor(color).class} p-6 rounded-xl border ${getNoteColor(color).border} ${className}`}>
            ...
            <div className="min-h-[300px] mb-8">
                <RichTextEditor
                    ...
                    className={`min-h-[300px] ${getNoteColor(color).class} border-none focus:ring-0`}
                />
            </div>

             {/* Settings Panel */}
             {isSettingsOpen && (
                <div className={`sticky bottom-0 z-10 pt-6 border-t border-gray-200/50 space-y-6 animate-in fade-in slide-in-from-top-2 ${getNoteColor(color).class} -mx-6 -mb-6 px-6 pb-6 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]`}>
                 {/* Color Picker */}
                <div>
                    ...
                    <div className="space-y-4">
                        ...
                        {isReminder && (
                            <div className="space-y-4 pt-2 animate-in fade-in slide-in-from-top-2">
                                ...
                                {/* Webhook Settings */}
                                <div>
                                    <label className="block text-xs font-medium text-gray-500 uppercase tracking-wide mb-1.5">Webhook URL (Optional)</label>
                                    <input
                                        type="url"
                                        placeholder="https://discord.com/api/webhooks/..."
                                        value={webhookUrl}
                                        onChange={(e) => setWebhookUrl(e.target.value)}
                                        className="block w-full px-3 py-2 text-sm border-gray-300 rounded-md focus:ring-primary focus:border-primary bg-white/50 placeholder-gray-400"
                                    />
                                </div>
